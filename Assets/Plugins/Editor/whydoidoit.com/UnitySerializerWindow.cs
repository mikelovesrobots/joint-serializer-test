...
public class UnitySerializerWindow : EditorWindow
{
	...
	{
		if (Selection.activeGameObject == null || Selection.activeGameObject.GetComponent<StoreInformation>() == null || Selection.activeGameObject.renderer == null)
			return;
					...
					using (new HorizontalCentered()) {
						using (new Vertical()) {
							using (new Horizontal()) {
								...
								if(Selection.activeGameObject.GetComponent<StoreMaterials>() == null && Selection.activeGameObject.GetComponent<StoreInformation>() != null && Selection.activeGameObject.renderer != null)
								{
						...
						using (new HorizontalCentered()) {
							using (new Vertical()) {
								...
								if (GUILayout.Button ("Remove All")) {
									...
									foreach (var c in Selection.activeGameObject.GetComponentsInChildren<UniqueIdentifier>())
										DestroyImmediate (c);
									foreach (var c in Selection.activeGameObject.GetComponentsInChildren<StoreMaterials>())
										DestroyImmediate (c);
								}
								GUI.color = Color.white;
							}
							...
							using (new Vertical()) {
								using (new Horizontal()) {
									if (GUILayout.Button ("Exclude from room")) {
										Undo.RegisterSceneUndo ("Change Identifiers");
										foreach (var c in Selection.activeGameObject.GetComponentsInChildren<Transform>().Select(t=>t.gameObject).Where(g=>!g.GetComponent<DontStoreObjectInRoom>()))
								...
								if (GUILayout.Button ("Remove")) {
									Undo.RegisterSceneUndo ("Change Identifiers");
									foreach (var c in Selection.activeGameObject.GetComponentsInChildren<DontStoreObjectInRoom>())
										DestroyImmediate (c);
									}
								GUI.color = Color.white;
							}
				...
				using (new HorizontalCentered()) {
					using (new Vertical()) {
						...
						using(new Horizontal())
						{
							...
							if (GUILayout.Button ("Remove")) {
								Undo.RegisterSceneUndo ("Change Identifiers");
								foreach (var c in Selection.gameObjects.Cast<GameObject>().SelectMany(s=>s.GetComponentsInChildren<DontStoreObjectInRoom>()))
									DestroyImmediate (c);
							}
							GUI.color = Color.white;
